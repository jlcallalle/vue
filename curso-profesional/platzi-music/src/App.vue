<template>
  ...
  <!-- <v-client-table :columns="columns" v-model="data" :options="options">
    ...
    <h1>{{ message }}</h1>
     ...
     <v-client-table v-model="data2" :columns="columns2" :options="options" >
      <!-- <a slot="uri" slot-scope="props" target="_blank" :href="props.row.uri" class="glyphicon glyphicon-eye-open"></a> -->
...
export default {
  ...
  methods: {
    clickedButton: function() {
			console.log(this.$refs, 'evento click button');
		}
  },
  mounted() {
		console.log( this.$refs , 'mounted load $refs ');
	}
}
